...
export default function Debts() {
  ...
  const debtSales = useMemo(() => {
    let filtered = sampleSales.filter(
      (s) =>
        s.store_id === activeStoreId &&
        (s.payment_status === "debt" || s.customer_id),
    );
    // Only keep sales that were ever debt
    filtered = filtered.filter(
      (s) => s.payment_status === "debt" || getTotalPaidForSale(s.id) > 0,
    );
    ...
    if (filterStatus === "paid")
      filtered = filtered.filter(
        (s) => s.payment_status === "paid" && getTotalPaidForSale(s.id) > 0,
      );
  ...
  const totalUnpaid = useMemo(
    () =>
      sampleSales
        .filter(
          (s) => s.store_id === activeStoreId && s.payment_status === "debt",
        )
  ...
  const unpaidCount = sampleSales.filter(
    (s) => s.store_id === activeStoreId && s.payment_status === "debt",
  ).length;
  ...
  const handleMarkPaid = () => {
    ...
    markDebtAsPaid(selectedSale.id, user.name || "Admin", user.role);
    setPayAmount("");
    setPayNote("");
    setSelectedSale(null);
    setRefreshKey((k) => k + 1);
            ...
            <div>
              ...
              {(() => {
                const nearest = sampleSales
                  ...
                  .sort(
                    (a, b) =>
                      (a.due_date?.getTime() || 0) -
                      (b.due_date?.getTime() || 0),
                  )[0];
                return nearest ? (
                  <>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {getCustomerName(nearest.customer_id)}
                    </p>
                  </>
                ) : (
                ...
                >
                  ...
                  <TableCell>
                    ...
                    <div className="text-xs text-muted-foreground">
                      {getCustomerPhone(sale.customer_id)}
                    </div>
                  </TableCell>
        ...
        <DialogContent className="max-w-lg">
          ...
          {selectedSale && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div>
                  ...
                  <p className="font-bold text-lg text-orange-600">
                    {formatCurrency(getRemainingDebt(selectedSale))}
                  </p>
                </div>
              </div>
              ...
              {(() => {
                const ship = getShipmentsForStore(activeStoreId).find(
                  (s) => s.sale_id === selectedSale.id,
                );
                if (!ship) return null;
                return (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {ship.recipient_address}
                    </p>
                    <p className="text-xs mt-1">
                      Ongkir: {formatCurrency(ship.shipping_cost)}
                    </p>
                  </div>
                );
              })()}
              ...
              {selectedSale.payment_status === "paid" &&
                selectedSale.paid_at && (
                  <div className="bg-green-50 border border-green-200 rounded-lg p-3 text-sm">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Ditandai oleh <strong>{selectedSale.paid_by_name}</strong>{" "}
                      ({selectedSale.paid_by_role}) pada{" "}
                      {formatDate(selectedSale.paid_at)}
                    </p>
                  </div>
                )}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          setPayAmount(String(getRemainingDebt(selectedSale)))
                        }
                      >
                        Bayar Lunas
                      </Button>
