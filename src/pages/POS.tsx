...
export default function POS() {
  ...
  const updateLocalProductStock = (soldItems: CartItem[]) => {
    const soldMap = new Map(
      soldItems.map(item => [item.product.id, item.quantity])
    );
    
    setStoreProducts(prev =>
      prev.map(product => {
        const qty = soldMap.get(product.id);
  ...
  const processOwnerWithdrawal = async () => {
    ...
    try {
      ...
      // Invoice number generated by database
      ...
      // Update local stock
      updateLocalProductStock(soldItems);
