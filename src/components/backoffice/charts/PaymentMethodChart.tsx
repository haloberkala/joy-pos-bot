...
export function PaymentMethodChart({ sales }: PaymentMethodChartProps) {
  const chartData = useMemo(() => {
    const salesByMethod: Record<string, { total: number; count: number }> = {};
    sales.forEach(s => {
      if (!salesByMethod[s.payment_method]) {
        salesByMethod[s.payment_method] = { total: 0, count: 0 };
      }
      salesByMethod[s.payment_method].total += s.grand_total;
      salesByMethod[s.payment_method].count += 1;
    });
  ...
  return (
    <Card className="p-4">
      <div className="mb-4">
        ...
        <p className="text-sm text-muted-foreground">{sales.length} transaksi</p>
      </div>
      <div className="grid grid-cols-3 gap-4 mb-4">
        {Object.entries(PAYMENT_LABELS).slice(0, 3).map(([key, { label, color, icon }]) => {
      ...
      <div className="h-[180px]">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} layout="vertical">
            ...
            <Tooltip contentStyle={{ backgroundColor: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: '8px' }} formatter={(value: number) => [formatCurrency(value), 'Total']} />
